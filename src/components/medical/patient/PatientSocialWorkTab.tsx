...
import {
    ...
    Sparkles, FileText, Heart, ChevronDown,
} from "lucide-react";

// ── Types ──────────────────────────────────────────────
//
// El tipo `Visit` (visitas de especialistas) se eliminó: el endpoint
// /api/social/visits NO existe — el modelo `SpecialistVisit` fue
// dropeado (task #22, drop quirúrgico) y reemplazado por el flujo
// de Servicios Externos (tablet de pisos + endpoint distinto). La
// visualización de visitas externas vive ahora en
// `PatientExternalServicesTab`, que se mantiene como pestaña aparte
// y NO se toca aquí.
...
export default function PatientSocialWorkTab({ patientId }: { patientId: string }) {
    ...
    // Sub-tabs (sin "specialists" — esa data vive en PatientExternalServicesTab)
    const [subTab, setSubTab] = useState<"notes" | "tasks" | "benefits">("notes");
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1 lg:w-3/5 space-y-4">
                {/* Sub-tabs (sin Especialistas — vive en PatientExternalServicesTab) */}
                        ...
                        {notes.length === 0 ? (
                            ...
                            <div key={n.id} className="bg-white border border-slate-100 rounded-xl p-4 space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    {(n.createdBy.id === user?.id || user?.role === "DIRECTOR" || user?.role === "ADMIN") && (
