            ...
            <div className="max-w-5xl mx-auto space-y-8">
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input type="text" value={supportSource} onChange={e => setSupportSource(e.target.value)} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium" placeholder="Ej: Hija y sobrino" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Fecha Inicio</label>
                                    <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium" />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Revisión Smt.</label>
                                    <input type="date" value={nextReview} onChange={e => setNextReview(e.target.value)} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium" />
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" value={cognitiveLevel} onChange={e => setCognitiveLevel(e.target.value)} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium" placeholder="Ej: Desorientación parcial" />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Movilidad</label>
                                    <input type="text" value={mobility} onChange={e => setMobility(e.target.value)} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium" placeholder="Ej: Deambula con andador" />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Continencia</label>
                                    <input type="text" value={continence} onChange={e => setContinence(e.target.value)} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium" placeholder="Ej: Incontinencia ocasional" />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Dieta</label>
                                    <input type="text" value={dietDetails} onChange={e => setDietDetails(e.target.value)} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium" />
                                </div>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Diagnósticos y Resumen</label>
                                <textarea value={clinicalSummary} onChange={e => setClinicalSummary(e.target.value)} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium h-20 resize-none" placeholder="Demencia vascular moderada, HTA..." />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
                    ...
                    <div className="space-y-3">
                        {risks.map((r, i) => (
                            <div key={i} className="flex flex-col md:flex-row gap-3 items-center bg-slate-50 p-2 border border-slate-200 rounded-xl relative">
                                <input className="w-full md:w-1/4 bg-white p-2 rounded-lg border font-bold text-sm" placeholder="Área (Ej. Seguridad)" value={r.area} onChange={e => { const copy = [...risks]; copy[i].area = e.target.value; setRisks(copy); }} />
                                <input className="w-full md:flex-1 bg-white p-2 rounded-lg border font-medium text-sm" placeholder="Hallazgo / Necesidad" value={r.finding} onChange={e => { const copy = [...risks]; copy[i].finding = e.target.value; setRisks(copy); }} />
                    ...
                    <div className="mt-4">
                        ...
                        <textarea value={interdisciplinarySummary} onChange={e => setInterdisciplinarySummary(e.target.value)} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium h-20 resize-none" placeholder="El residente conserva capacidad parcial para..." />
                    </div>
                </div>
                    ...
                    <div className="space-y-3">
                        {goals.map((g, i) => (
                            <div key={i} className="flex flex-col gap-2 bg-indigo-50/50 p-4 border border-indigo-100 rounded-xl relative">
                                ...
                                <textarea className="w-full bg-white p-2.5 rounded-lg border font-medium text-sm h-16 resize-none" placeholder="Acción / Intervención Concreta" value={g.action} onChange={e => { const copy = [...goals]; copy[i].action = e.target.value; setGoals(copy); }} />
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                    <div><label className="text-[10px] font-bold text-slate-500 uppercase">Responsable</label><input className="w-full bg-white p-2 rounded-lg border text-sm font-bold" value={g.responsible} onChange={e => { const copy = [...goals]; copy[i].responsible = e.target.value; setGoals(copy); }} /></div>
                                    <div><label className="text-[10px] font-bold text-slate-500 uppercase">Frecuencia</label><input className="w-full bg-white p-2 rounded-lg border text-sm font-medium" value={g.frequency} onChange={e => { const copy = [...goals]; copy[i].frequency = e.target.value; setGoals(copy); }} /></div>
                    ...
                    <div className="space-y-3">
                        {recommendedServices.map((rs, i) => (
                            <div key={i} className="flex flex-col gap-2 bg-white p-4 border border-orange-200 rounded-xl relative shadow-sm">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                    <div className="md:col-span-2"><label className="text-[10px] font-bold text-orange-700 uppercase">Descripción de los Beneficios</label><input className="w-full bg-slate-50 p-2 rounded-lg border text-sm font-medium" value={rs.description} onChange={e => { const copy = [...recommendedServices]; copy[i].description = e.target.value; setRecommendedServices(copy); }} /></div>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <textarea value={familyEducation} onChange={e => setFamilyEducation(e.target.value)} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium h-24 resize-none" placeholder="Se orienta sobre prevención de caídas..." />
                        </div>
                        <div>
                            <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Preferencias del Residente</label>
                            <textarea value={preferences} onChange={e => setPreferences(e.target.value)} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium h-24 resize-none" placeholder="Música suave, rutinas consistentes..." />
                        </div>
                        <div>
                            <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Método de Monitoreo</label>
                            <textarea value={monitoringMethod} onChange={e => setMonitoringMethod(e.target.value)} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium h-20 resize-none" placeholder="Notas de enfermería, reportes zendity..." />
                        </div>
                        <div>
                            <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Criterios de Revisión Temprana</label>
                            <textarea value={revisionCriteria} onChange={e => setRevisionCriteria(e.target.value)} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium h-20 resize-none" placeholder="Hospitalización, deterioro funcional..." />
                        </div>
                    </div>
                </div>
