        ...
        {showPublishModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
                    ...
                    {/* Header — DOS estados claramente diferenciados:
                          - publishErrors > 0   → ROJO bloqueo real
                          - publishWarnings > 0 → TEAL informativo (NO bloqueo)
                        Antes ambos casos usaban amber + "Revisar antes" + botón "de todas formas"
                        → la directora confundía warnings con errores y cancelaba.
                        2-jun-2026: rediseño completo. */}
                    {publishErrors.length > 0 ? (
                        ...
                        <div className="bg-teal-50 px-6 py-4 border-b border-teal-100 flex items-center gap-3">
                            <div className="w-10 h-10 bg-[var(--color-zendity-teal)] text-white rounded-xl flex items-center justify-center text-lg font-black shrink-0">✓</div>
                            <div>
                                <h3 className="font-black text-slate-800">
                                    Listo para publicar
                                    {publishWarnings.length > 0 && <span className="text-slate-500 font-bold"> · {publishWarnings.length} aviso{publishWarnings.length !== 1 ? 's' : ''} menor{publishWarnings.length !== 1 ? 'es' : ''}</span>}
                                </h3>
                                <p className="text-xs font-bold text-[var(--color-zendity-teal)]">
                                    {publishWarnings.length > 0 ? 'Revisa los avisos y publica con un solo click.' : 'Sin avisos. Publica cuando estés lista.'}
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
                        ...
                        {/* Avisos menores — NO son errores, son recordatorios informativos */}
                        {publishWarnings.length > 0 && publishErrors.length === 0 && (
                            <div className="space-y-2">
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                    Avisos menores — informativos, NO bloquean
                                </p>
                                {publishWarnings.map((w, i) => (
                                    <div key={i} className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 font-medium leading-relaxed">
                                        {w.message}
                                    </div>
                                ))}
                                <p className="text-[11px] text-slate-500 italic mt-2">
                                    Estos avisos son normales. Puedes publicar con confianza.
                                </p>
                            </div>
                        )}
                    </div>

                    {/* Footer buttons — jerarquía visual clara:
                        publish primary (grande, verde) · cancelar secundario (link) */}
                    <div className="px-6 py-4 border-t border-slate-100">
                        {publishErrors.length > 0 ? (
                            <button
                                onClick={() => { setShowPublishModal(false); setPublishErrors([]); setPublishWarnings([]); }}
                                className="w-full py-3 text-sm font-bold text-white bg-rose-600 hover:bg-rose-700 rounded-xl transition-colors"
                            >
                                Cerrar y corregir
                            </button>
                        ) : (
                            <div className="flex flex-col items-center gap-3">
                                <button
                                    ...
                                    disabled={publishing}
                                    className="w-full py-3.5 text-base font-black text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl transition-colors disabled:opacity-50 shadow-sm shadow-emerald-600/20 flex items-center justify-center gap-2"
                                >
                                    {publishing ? (
                                        <>Publicando...</>
                                    ) : (
